...
def create_report_fpdf(data, model, adv_val_result, ks_result):
...
def get_performance_metric(preds: pd.DataFrame):
    ...
    Returns:
        pd.DataFrames 
    """
    X_train = preds[preds['is_train'] == 1].copy()
    X_test = preds[preds['is_train'] == 0].copy()
    target = 'target'
    ...
    general_info = {
        ...
        'Total good': X_train[target].value_counts()[0] + X_test[target].value_counts()[0],
        'Total bad': X_train[target].value_counts()[1] + X_test[target].value_counts()[1],
        'Train good': X_train[target].value_counts()[0], 
        'Train bad': X_train[target].value_counts()[1], 
        'Test good': X_test[target].value_counts()[0], 
        'Test bad': X_test[target].value_counts()[1], 
        'Train NPL': round(X_train[target].value_counts()[1] / X_train.shape[0], 4),
        'Test NPL': round(X_test[target].value_counts()[1] / X_test.shape[0], 4)
    }
    
    model_performance = {
        'Train AUC': round(roc_auc_score(X_train[target], X_train['predictions']), 3),
        'Test AUC': round(roc_auc_score(X_test[target], X_test['predictions']), 3),
        'Train Gini': round(2*round(roc_auc_score(X_train[target], X_train['predictions']), 3)-1, 3),
        'Test Gini': round(2*round(roc_auc_score(X_test[target], X_test['predictions']), 3)-1, 3)
    }
    
    for thres in [x / 100 for x in range(5, 51, 5)]:
        thres_metrics = dict()
        
        X_test['prediction_label'] = (X_test['predictions'] > thres).astype(int)
        
        thres_metrics['Threshold'] =    thres
        thres_metrics['Accuracy'] =     round(accuracy_score(X_test[target], X_test['prediction_label']), 3)
...
def get_feature_report(data: pd.DataFrame):
    ...
    for column in settings.SET_FEATURES.features_list:
        if column in list(data.columns):
            ...
            feature_info.append(column_info)
